...
The code for this guide is available in [GitHub][spring-security-oauth2-authorization-server.git].

### Create a new project
Create a new Spring Boot project using Gradle.

```bash
~$ mkdir spring-security-oauth2-authorization-server && cd spring-security-oauth2-authorization-server
~$ gradle init --project-name spring-security-oauth2-authorization-server --type java-application --test-framework junit --package spring.security.oauth2.authorization.server --dsl groovy
```
...
Finally, remove the `src/test/java/spring/security/oauth2/resource/server/AppTest.java`.
...
The complete, working solution is available in [GitHub][spring-security-oauth2-authorization-server.git].
